...
18.4 × 12.1 × 5.8 m. A faceted armoured central hull: every facet of the
cheeks, the forward body and the aft module carries its own bolted plate,
the gunmetal hull showing in the gaps (a few plates left off over machinery);
...
ribbed nozzles on the aft flanks. Amber accent strips. On the spine: the
comms mast, a whip antenna, the dish on a gimbal and a camera turret. Three
chunky hydraulic legs with big pads on their own node (`Gear`, hidden in
flight). Markings: STELLAR on the cheeks and pods, the full five-cross
...
# Stations close enough that every facet is one armour plate (about a metre
# and a half), so the plating reads as bolted-on panels.
STATIONS = [
...
def mats():
    return {
        'white': hs.paint('Plating', WHITE, seed=1.3, wear=0.55, grime=0.55, panel=(1.45, 1.05, 0.85), seam=0.03, patches=0.12, pits=0.42, pit_scale=1.9),
        'white2': hs.paint('PlatingPods', (0.64, 0.63, 0.6, 1), seed=2.7, wear=0.55, grime=0.55, panel=(1.2, 0.95, 0.8), seam=0.03, patches=0.12, pits=0.42, pit_scale=1.9),
...
def hull(p, m):
    ...
    for pred in (
        lambda c, n: 0.4 < c.y < 2.6 and abs(n.y) < 0.5,
    ...
    armoured = faces_where(obj, lambda c, n: (-6.2 < c.y < 0.4 or 2.6 < c.y < 5.0) and abs(n.y) < 0.5)
    set_faces(obj, armoured, 3)
    armour = plates(obj, armoured, 'Armour', PLATE, 0.045, skip=0.08, seed=3,
                    keep=lambda c: abs(c.z - HZ) < 0.6 and -2.2 < c.y < -1.0 or c.z > TOP - 0.1)
    model.bevel(obj, 0.045 if p.detail else 0.03, p.seg, 28)
    p.items.append((obj, 0))
    if p.detail:
        model.bevel(armour, 0.02, 2, 30)
    p.add(armour, m['white'])
    ...
    def band_spot(rnd):
        s = rnd.choice((1, -1))
        side = rnd.random() < 0.6
        y = rnd.uniform(0.55, 2.45) if rnd.random() < 0.55 else rnd.uniform(5.15, 6.85)
        if side:
...
def pod(p, m, s):
    ...
    sleeve_faces = faces_where(obj, lambda c, n: 0.2 < c.y < 1.8 and abs(n.y) < 0.5)
    set_faces(obj, sleeve_faces, 1)
    armoured = faces_where(obj, lambda c, n: (-5.3 < c.y < 0.2 or 1.8 < c.y < 4.3) and abs(n.y) < 0.5)
    set_faces(obj, armoured, 2)
    armour = plates(obj, armoured, 'PodArmour', PLATE, 0.045, skip=0.06, seed=7 + s,
                    keep=lambda c: abs(c.x - x) > POD_FLAT - 0.1 and abs(c.z - POD_Z) < 0.5,
                    omit=lambda c: -5.3 < c.y < -4.2 and abs(c.x - x) > POD_FLAT - 0.1 and abs(c.z - POD_Z) < 0.5)
    model.bevel(obj, 0.04 if p.detail else 0.028, p.seg, 30)
    p.items.append((obj, 0))
    if p.detail:
        model.bevel(armour, 0.02, 2, 30)
    p.add(armour, m['white2'])
    # A ribbed machinery sleeve round the pod's waist, proud of the plating.
    sr = POD_R + 0.08
    sleeve = [(POD_FLAT - 0.02, 0.15), (sr, 0.25)]
    k = 5 if p.detail else 3
    for i in range(k):
        t = 0.35 + i * (1.2 / k)
        sleeve += [(sr, t), (sr + 0.05, t + 0.03), (sr + 0.05, t + 0.14), (sr, t + 0.17)]
    sleeve += [(sr, 1.7), (POD_FLAT - 0.02, 1.85)]
    p.add(hs.lathe('Sleeve', sleeve, 32 if p.detail else 20, location=(x, 0, POD_Z)), m['ribs'])
    engine(p, m, 'PodEngine', x, POD_Z, POD_ENG_R, 4.2, 6.9)
...
def gear(p, m):
    ...
    for mount, knee, ankle, s in legs:
        ...
        p.add(model.bevel(model.box('Pad', (1.3, 1.2, 0.2), (ankle.x, ankle.y, -3.02)), 0.04, p.seg), m['dark'], node=1)
        p.add(model.box('PadRim', (1.14, 1.04, 0.06), (ankle.x, ankle.y, -2.9)), m['steel'], node=1)
        p.add(model.box('PadStrip', (0.6, 0.08, 0.03), (ankle.x, ankle.y - 0.62, -2.98)), m['amber'], node=1)
