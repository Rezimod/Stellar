...
import { armLongitude, gauss, pickArm, ANDROMEDA_MODEL, MILKY_WAY_MODEL, type SpiralModel } from '@/lib/solar-system/galaxy-paint';
import { galaxyDiskTexture } from '@/lib/solar-system/galaxy-disk';
...
export function makeMilkyWayDisk(lite = false): MilkyWayHandle {
  ...
  galaxy.add(disk);
  // Painted in a worker from the start (galaxy-disk.ts), so it is there by
  // the time anyone gets this far out. Until then a blank stands in, which
  // keeps the program the one the painting will use: no compile on arrival.
  const blankDisk = new THREE.DataTexture(new Uint8Array(4), 1, 1);
  blankDisk.needsUpdate = true;
  diskMat.map = blankDisk;
  let diskPainted = false;
  let diskGone = false;
  void galaxyDiskTexture('milky-way', lite).then((tex) => {
    if (diskGone) { tex.dispose(); return; }
    diskMat.map = tex;
    diskPainted = true;
  });
  ...
  const setFade = (fade: number) => {
    ...
    // Until its painting arrives the disk is not drawn; the stars carry it.
    diskMat.opacity = diskPainted ? f * 0.78 : 0;
...
export function makeAndromedaGalaxy(lite: boolean): AndromedaHandle {
  ...
  let diskPainted = false;
  let diskGone = false;
  void galaxyDiskTexture('andromeda', lite).then((tex) => {
    if (diskGone) { tex.dispose(); return; }
    diskMat.map = tex;
    diskPainted = true;
  });
  ...
  const setFade = (fade: number) => {
    const f = THREE.MathUtils.clamp(fade, 0, 1);
    diskMat.opacity = diskPainted ? f * 0.8 : 0;
    mat.opacity = f * 0.85;
