  ...
  return (
    <div className="w-full flex flex-col gap-3">
      <div className="flex items-center gap-2 flex-wrap">
        <p className="text-white font-semibold" style={{ fontFamily: 'Georgia, serif' }}>Tonight's Targets</p>
        {streak && streak.streak > 0 && (
          <>
            <span className="text-amber-400 text-xs ml-1">🔥 {streak.streak} day streak</span>
            {streak.bonusStars > 0 && (
              <span className="text-[#FFD166] text-xs">+{streak.bonusStars} ✦ bonus</span>
            )}
          </>
        )}
      </div>
      ...
      <div className="flex flex-col gap-2.5">
        {targets.map(t => (
          <button
            ...
            className="flex items-center gap-3 rounded-xl p-4 text-left w-full transition-colors hover:bg-white/[0.04]"
            style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}
          >
            <span className="text-2xl flex-shrink-0">{t.emoji}</span>
            <div className="flex-1 min-w-0">
              <p className="text-white font-semibold text-sm">{t.name}</p>
              <p className="text-slate-400 text-xs mt-0.5 line-clamp-1">{t.description}</p>
