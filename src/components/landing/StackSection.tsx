...
export default function StackSection() {
  return (
    <section className="py-16 md:py-24 bg-white/[0.012] border-t-[0.5px] border-[rgba(var(--ink-warm),0.06)]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {CARDS.map((c) => (
          <div
            key={c.tag}
            className="rounded-[10px] border-[0.5px] border-[rgba(var(--ink-warm),0.08)] p-[14px] md:p-[18px]"
          >
            <div className="font-mono text-[9px] tracking-[0.18em] text-[rgba(var(--ink-warm),0.4)]">
              {c.tag}
            </div>
            <div className="mt-2 font-display text-[15px] md:text-[16px] text-[#F8F4EC]">
              {c.name}
            </div>
            <p className="mt-1.5 text-[11px] leading-[1.5] text-[rgba(var(--ink-warm),0.5)]">
              {c.desc}
            </p>
          </div>
        ))}
