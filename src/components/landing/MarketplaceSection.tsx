    ...
    <section className="py-16 md:py-24">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-[1fr_1.1fr] gap-8 md:gap-12 items-center">
        <div className="md:order-2">
          ...
          <Link
            href="/marketplace"
            className="mt-6 inline-flex items-center justify-center rounded-lg border-[0.5px] border-[rgba(255,209,102,0.3)] bg-[rgba(255,209,102,0.12)] px-5 py-2.5 text-[12px] font-medium text-[#FFD166] transition-colors hover:bg-[rgba(255,209,102,0.18)]"
          >
            Browse marketplace  →
          </Link>
        </div>
        ...
        <div className="md:order-1 rounded-[14px] border-[0.5px] border-[rgba(232,230,221,0.08)] bg-[rgba(232,230,221,0.025)] p-[18px] md:p-[22px]">
          ...
          <div className="mt-3 flex items-center justify-between rounded-[10px] border-[0.5px] border-[rgba(132,101,203,0.2)] bg-[rgba(132,101,203,0.06)] p-[14px]">
            <div className="min-w-0">
              <div className="font-serif text-[12px] md:text-[13px] text-[#E8E6DD] truncate">
            ...
            <div className="shrink-0 text-[11px] text-[#FFD166] font-mono">680 GEL</div>
          </div>
          ...
          <div className="mt-4 pt-3.5 flex justify-between items-center text-[11px] border-t-[0.5px] border-[rgba(232,230,221,0.08)]">
            ...
            <span className="text-[#FFD166] font-mono">485 ✦  →  10% off</span>
          </div>
        </div>
      </div>
    </section>
...
function ProductCard({
  ...
  const styles =
    tone === 'gold'
      ? {
          ...
          chipInner: <span className="block w-3.5 h-3.5 rounded-full bg-[#FFD166]" />,
        }
      : {
          wrap: 'border-[rgba(56,240,255,0.15)] bg-[rgba(56,240,255,0.04)]',
          chipOuter: 'bg-[rgba(56,240,255,0.10)]',
          chipInner: <span className="block w-3.5 h-3.5 rounded-full border border-[#38F0FF]" />,
        };
  ...
  return (
    <div className={`flex-1 min-w-0 rounded-[10px] border-[0.5px] p-[12px] md:p-[14px] ${styles.wrap}`}>
      ...
      <div className="mt-3 font-serif text-[12px] md:text-[13px] text-[#E8E6DD] truncate">
        {title}
      </div>
      <div className="mt-0.5 text-[10px] text-[rgba(232,230,221,0.5)]">{subtitle}</div>
      <div className="mt-2 text-[11px] font-medium text-[#FFD166] font-mono">{price}</div>
    </div>
  );
}
