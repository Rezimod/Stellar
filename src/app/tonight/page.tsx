...
export default async function TonightPage() {
  ...
  const nightLabel = (night: string) =>
    new Intl.DateTimeFormat('en-GB', { timeZone: 'UTC', day: 'numeric', month: 'long' }).format(new Date(`${night}T00:00:00Z`));

  const d = view?.decided ?? null;
  const lastLost = view?.recent.find((r) => r.night === addDays(view.night, -1) && r.result === 'lost') ?? null;
  const art = d ? (SET_001_CARD_BY_DESIGNATION.get(d.designation)?.seed.artUrl ?? d.artUrl) : null;

  const plan: Datum[] = d
    ? [
        { label: 'Planned', value: d.plannedAt ? `${localTime(d.plannedAt)} local` : '—' },
        { label: 'Cloud', value: d.cloudForecast === null ? 'No forecast' : `${d.cloudForecast}% forecast` },
  ...
  return (
    <SideraShell>
      ...
      <section className="sd-hero" style={{ paddingBottom: 24 }}>
        ...
        <div className="sd-container">
          <p className="sd-eyebrow">
            Node 01 · {node.site} · {view ? `Night of ${nightLabel(view.night)}` : 'Tonight'}
          </p>
          <h1 className="sd-display" style={{ maxWidth: '14ch' }}>
            {d ? d.name : 'Not yet decided.'}
          </h1>
          <p className="sd-hero__sub">
            {d ? d.basis : 'Decided at 17:00 Tbilisi time, from the holders’ votes.'}
          </p>
          {lastLost && (
            <p className="sd-data" style={{ marginTop: 16 }}>
              The night of {nightLabel(lastLost.night)} was lost to cloud. {lastLost.lostReason}
            </p>
          )}
        </div>
      </section>
              ...
              {d.capture ? (
                <div className="sd-section">
                  ...
                  <DataRow
                    layout="stacked"
                    items={[
                      { label: 'UTC', value: d.capture.capturedAt.slice(0, 16).replace('T', ' ') },
      ...
      {view && (
        <section className="sd-container sd-section">
          <h2 className="sd-section__title">Vote · night of {nightLabel(view.voting.night)}</h2>
          {view.voting.carried ? (
            <p className="sd-note">
