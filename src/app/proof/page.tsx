...
function ProofCard({ mission, onDelete }: { mission: CompletedMission; onDelete: () => void }) {
  ...
  return (
    <div className={`glass-card rounded-xl overflow-hidden flex flex-col ${isPending ? '!border-terracotta' : ''}`} style={isPending ? { borderColor: 'rgba(227, 218, 201,0.5)' } : {}}>
        ...
        <div className="flex gap-2 mt-auto pt-2">
          {typeof navigator !== 'undefined' && 'share' in navigator && (
            <button
              ...
              className="text-xs px-2 py-1.5 border border-[rgba(227, 218, 201,0.12)] hover:border-[var(--terracotta)] text-text-muted hover:text-[var(--terracotta)] rounded transition-all flex items-center justify-center gap-1"
          ...
          {isRealTx ? (
            <button
              ...
              className="flex-1 text-center text-xs px-2 py-1.5 border border-[rgba(227, 218, 201,0.12)] hover:border-[var(--terracotta)] text-text-muted hover:text-[var(--terracotta)] btn-glow-cyan rounded transition-all flex items-center justify-center gap-1"
            >
            ...
            <button
              ...
              className="flex-1 text-center text-xs px-2 py-1.5 border border-[rgba(227, 218, 201,0.12)] text-text-muted opacity-50 cursor-not-allowed rounded transition-all flex items-center justify-center gap-1"
            >
          ...
          {!confirming ? (
            <button
              ...
              style={{ border: '1px solid rgba(227, 218, 201,0.12)' }}
            >
            ...
            <div className="flex-1 flex gap-1">
              ...
              <button onClick={() => setConfirming(false)} className="flex-1 text-xs px-2 py-1.5 text-text-muted rounded" style={{ border: '1px solid rgba(227, 218, 201,0.12)' }}>Cancel</button>
            </div>
          )}
        </div>
      </div>
...
export default function ProofPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-12 animate-page-enter">
      ...
      {proofs.length === 0 ? (
        <div className="text-center py-20 rounded-xl" style={{ border: '1px dashed rgba(227, 218, 201,0.12)' }}>
