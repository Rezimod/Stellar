...
import {
  UP_NOW_STARS_BASE, UP_NOW_STARS_STREAK_BONUS, UP_NOW_STREAK_BONUS_THRESHOLD, UP_NOW_ROUNDS_PER_DAY,
} from '@/lib/games/up-now';

const GAME_ID = 'up_now';

// Server always derives "today" itself (UTC) — the client's utcDate is
// informational only, never trusted for the once-per-day gate or streak.
...
export async function POST(req: NextRequest) {
  ...
  const score = Math.max(0, Math.min(UP_NOW_ROUNDS_PER_DAY, Number.isInteger(body.score) ? (body.score as number) : 0));
  ...
  // Claim a slot in the shared Stars ledger first — this is what makes the
  // Up Now award count against the same daily/monthly issuance cap as every
  // other reward path (quizzes, finds, daily check-in, observations).
  let claimed = false;
  try {
    ...
    // Already played today — return the existing result rather than erroring.
    ...
    return NextResponse.json({
      score: row?.score ?? 0,
      streak: null,
      starsAwarded: row?.stars ?? 0,
      alreadyPlayed: true,
    });
  }

  // Streak: consecutive UTC days ending today, from this wallet's prior plays.
  const priorRows = await db
    .select({ d: gameDailyPlays.utcDate })
    .from(gameDailyPlays)
    .where(and(eq(gameDailyPlays.wallet, wallet), eq(gameDailyPlays.game, GAME_ID)));
  const priorDates = priorRows.map((r) => r.d).filter((d): d is string => typeof d === 'string');
  const streak = streakFromDates([...priorDates, utcDate], utcDate);

  const baseAmount = streak >= UP_NOW_STREAK_BONUS_THRESHOLD ? UP_NOW_STARS_STREAK_BONUS : UP_NOW_STARS_BASE;
  ...
  try {
    ...
    return NextResponse.json({ score, streak, starsAwarded: amount, alreadyPlayed: false });
  } catch (err) {
    console.error('[games/up-now/complete]', err);
    // Release both rows so a genuine retry (not a duplicate) can re-claim.
    try {
